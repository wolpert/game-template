...
**Screens are Scene2D-driven.** `TheGame` now extends libGDX's `Game` and just bootstraps the Dagger graph + hands off to `ScreenNavigator`. Screens (`LoadingScreen`, `MainMenuScreen`, `PreferencesScreen`, `LevelPickerScreen`) extend `BaseScreen` (Stage + viewport + boilerplate); `GameScreen` implements `Screen` directly because gameplay is Ashley/Box2D-driven, not Scene2D — Scene2D is reserved for menus. **Cycle break:** `ScreenNavigator` depends on every screen directly so it can `disposeAll()` them at shutdown; screens take `Provider<ScreenNavigator>` so Dagger can construct screens before the navigator. The `Game` instance itself is bound into the graph via `@BindsInstance` on the component builder. UI uses `assets/ui/uiskin.json` (bundled libGDX skin), provided as `@Singleton Skin` in `GameModule`. Adding a screen = new class + new field on `ScreenNavigator` + new `goToXxx()` + line in `disposeAll()`. **Game-state continuity:** `GameScreen` builds entities once via an `entitiesBuilt` flag — leaving and re-entering the screen resumes the existing simulation. Reset on entry would be a one-line change (clear the engine in `show()`), but continue is the simpler default.
