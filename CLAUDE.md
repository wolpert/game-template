...
**Dodging gameplay loop.** `BlockSpawnSystem` drops a new dynamic block every 1.5s via `FallingBlockFactory` (random x, slight initial angular velocity); `GameContactListener` flips `GameState.gameOver` when an entity with `InputComponent` touches one with `BlockComponent`; `GameScreen.render` updates the elapsed-time score (HUD label in top-left), navigates to `GameOverScreen` on `gameOver`. Restart path: `GameOverScreen → goToGame()` → `GameScreen.show` rebuilds from scratch (clears entities, destroys world bodies, `state.reset()`, `spawner.reset()`). `BlockSpawnSystem.timer` is `@Singleton` state so explicit `reset()` is required on session restart — don't rely on construction order.
